...
import { parseFile, saveFile, readFile, deleteFile, MAX_FILE_SIZE, MAX_PROJECT_SIZE, projectFileDir } from "../lib/files.js";
import { saveImageWithVariants, deleteImageVariants, variantPath, variantMime, type ImageResolution } from "../lib/images.js";
...
projectsRouter.get("/:id", requireAuth, (req, res) => {
  const user = (req as any).user;
  try { requireProjectMember(req.params["id"] as string, user.id); } catch (e) { return sendErr(res, e); }
  ...
  const files = db
    .prepare(
      `SELECT id, name, mime_type, size_bytes, width, height, has_variants, uploaded_by, uploaded_at
...
projectsRouter.patch("/:id", requireAuth, (req, res) => {
  ...
  try { member = requireProjectMember(req.params["id"] as string, user.id); } catch (e) { return sendErr(res, e); }
...
projectsRouter.delete("/:id", requireAuth, (req, res) => {
  const user = (req as any).user;
  try { requireProjectOwner(req.params["id"] as string, user.id); } catch (e) { return sendErr(res, e); }
...
projectsRouter.post("/:id/invites", requireAuth, (req, res) => {
  ...
  const expires_at = typeof expiresInHours === "number" && expiresInHours > 0 ? Date.now() + expiresInHours * 3_600_000 : null;
...
projectsRouter.post("/join/:token", requireAuth, (req, res) => {
  ...
  try {
    const tx = db.transaction(() => {
      const invite = db.prepare(`SELECT * FROM project_invites WHERE token = ?`).get(token) as any;
      ...
      const existing = db.prepare(`SELECT 1 FROM project_members WHERE project_id = ? AND user_id = ?`).get(invite.project_id, user.id);
      if (existing) throw Object.assign(new Error("already a member"), { status: 409 });

      const claim = db.prepare(`UPDATE project_invites SET used_count = used_count + 1 WHERE token = ? AND used_count < max_uses`).run(token);
      if (claim.changes === 0) throw Object.assign(new Error("invite exhausted"), { status: 410 });

      db.prepare(`INSERT INTO project_members (project_id, user_id, role, added_at) VALUES (?, ?, 'member', ?)`).run(invite.project_id, user.id, Date.now());
...
projectsRouter.delete("/:id/members/:userId", requireAuth, (req, res) => {
  ...
  if (targetUserId !== user.id) {
    try { requireProjectOwner(projectId, user.id); } catch (e) { return sendErr(res, e); }
  } else {
    if (!getMember(projectId, user.id)) return res.status(403).json({ error: "not a member" });
  ...
  const result = db.prepare(`DELETE FROM project_members WHERE project_id = ? AND user_id = ?`).run(projectId, targetUserId);
...
// POST /projects/:id/files — upload file (image → generate variants)
projectsRouter.post("/:id/files", requireAuth, upload.single("file"), async (req, res) => {
  const user = (req as any).user;
  const projectId = req.params["id"] as string;

  try { requireProjectMember(projectId, user.id); } catch (e) { return sendErr(res, e); }
  ...
  try {
    ...
    if (parsed.isImage) {
      ...
      const v = await saveImageWithVariants(buffer, dir, fileId, parsed.mimeType);
      storagePath = v.original.path;
      width = v.original.width;
      height = v.original.height;
      hasVariants = 1;
    } else {
      storagePath = saveFile(projectId, fileId, buffer);
    }

    const result = db.transaction(() => {
      db.prepare(
        `INSERT INTO project_files (id, project_id, name, mime_type, size_bytes, storage_path, text_content, width, height, has_variants, uploaded_by, uploaded_at)
         VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`
      ).run(fileId, projectId, parsed.normalizedName, parsed.mimeType, size, storagePath, parsed.textContent ?? null, width, height, hasVariants, user.id, now);
      return db.prepare(`SELECT id, project_id, name, mime_type, size_bytes, width, height, has_variants, uploaded_by, uploaded_at FROM project_files WHERE id = ?`).get(fileId);
    })();
    ...
    try {
      if (hasVariants) deleteImageVariants(storagePath);
      else if (storagePath) deleteFile(storagePath);
    } catch {}
    return sendErr(res, e);
  }
});
...
projectsRouter.get("/:id/files", requireAuth, (req, res) => {
  ...
  const files = db
    .prepare(`SELECT id, project_id, name, mime_type, size_bytes, width, height, has_variants, uploaded_by, uploaded_at FROM project_files WHERE project_id = ? ORDER BY uploaded_at DESC`)
    .all(projectId);
  res.json(files);
});

// GET /projects/:id/files/:fileId — download (original)
projectsRouter.get("/:id/files/:fileId", requireAuth, (req, res) => {
  ...
  let buffer: Buffer;
  try { buffer = readFile(file.storage_path); } catch { return res.status(404).json({ error: "file data not found on disk" }); }
...
// GET /projects/:id/files/:fileId/variant/:res — stream resized variant (images only)
projectsRouter.get("/:id/files/:fileId/variant/:res", requireAuth, (req, res) => {
  const user = (req as any).user;
  const projectId = req.params["id"] as string;
  const fileId = req.params["fileId"] as string;
  ...
  const file = db.prepare(`SELECT name, mime_type, storage_path, has_variants FROM project_files WHERE id = ? AND project_id = ?`).get(fileId, projectId) as any;
  if (!file) return res.status(404).json({ error: "file not found" });
  if (!file.has_variants && resolution !== "original") return res.status(404).json({ error: "no variants" });

  const p = variantPath(file.storage_path, resolution);
  if (!fs.existsSync(p)) return res.status(404).json({ error: "variant missing" });
  res.setHeader("Content-Type", variantMime(file.mime_type, resolution));
...
projectsRouter.delete("/:id/files/:fileId", requireAuth, (req, res) => {
  ...
  let member: { role: string };
  try { member = requireProjectMember(projectId, user.id); } catch (e) { return sendErr(res, e); }
  ...
  if (file.has_variants) deleteImageVariants(file.storage_path);
  else deleteFile(file.storage_path);
...
projectsRouter.patch("/chats/:chatId", requireAuth, (req, res) => {
  ...
  const chat = db.prepare(`SELECT * FROM chats WHERE id = ?`).get(req.params["chatId"] as string) as any;
  if (!chat) return res.status(404).json({ error: "chat not found" });
  if (chat.user_id !== user.id) return res.status(403).json({ error: "not your chat" });

  if (projectId !== undefined && projectId !== null) {
    if (!getMember(projectId, user.id)) return res.status(403).json({ error: "not a member of target project" });
  }

  db.prepare(`UPDATE chats SET project_id = ?, updated_at = ? WHERE id = ?`).run(projectId ?? null, Date.now(), req.params["chatId"] as string);
