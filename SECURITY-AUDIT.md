...
### 1. Invite/Token коды предсказуемы (Math.random)
...
### 3. Race condition на балансе → бесплатный Opus
...
### 4. Race condition на invite/token redemption
...
### 5. Нет rate limiting / brute-force protection
...
### 6. Whitelist моделей отсутствует
...
### 7. Утечка ошибок upstream в клиент
...
### 8. Нет лимита длины пароля → bcrypt DoS
...
### 9. JSON body 2 MB → DoS / сжигание квоты
...
### 11. NODE_ENV-зависимый `secure` cookie
...
### 14. Нет защиты от удаления последнего админа
...
### 15. Истёкшие сессии не чистятся
...
### 17. Нет аудит-лога админских действий
ban/suspend/grant tokens/delete user — никуда не пишутся. При продаже бизнесу обязательно (compliance + расследования).

**Фикс:** таблица `admin_audit_log (admin_id, action, target_id, payload, created_at)`.
...
### 20. Helmet / security headers отсутствуют
